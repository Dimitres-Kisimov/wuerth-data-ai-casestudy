...
- **Progress since — partial, and still a gap.** `agentic-automation-lab` now ships **approval gates with resumable state**, priced *per placement* on the reliability benchmark's own fault schedule: the agent stops, hands out a resumable digest-stamped envelope of its run, and resuming costs **0 extra tokens**. Risk-gated review — only the runs where the guards and the verifier disagree — takes silent-wrong quotes **11,523/yr → 0** for a **modelled** EUR 88,948/yr of review labour (**EUR 7.65 per prevented error**), while approving every step costs a **modelled** EUR 349,708/yr and buys nothing extra on that schedule *(synthetic data; assumed review times, labelled)*. The honest half is what is still missing: the reviewer is **modelled, and modelled generously** — an ideal one, so every catch rate is an **upper bound** and the zero false-alarm rate says nothing about real people — there is **no queue model**, and the audit-logging and confidence-threshold halves of this item are not built. Placement is not a free choice either: a *skipped validation* fault is **missed** at pre-commit and at every-step, and caught only at pre-delivery / risk-gated.
